...
def write_dataset(input_file, horizon, save=True):
    ...
    features = np.zeros((len(odometry) - horizon - 1,  horizon, 8), dtype=np.double)
    ...
    for i in tqdm(range(len(throttle_cmd) - horizon), desc="Compiling dataset"):
        features[i] = np.array([*odometry[i:i+horizon].T, throttle_cmd[i:i+horizon], steering_cmd[i:i+horizon], vrefs[i:i+horizon]]).T
